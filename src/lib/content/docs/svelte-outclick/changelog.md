...
- [breaking] Added new `fullClick` prop. Now clicking outside requires `pointerdown` and `pointerup` to fire at the outside of your element. Set it to `false` so `pointerdown` can fire the `outclick` event on its own.
- Fixed the empty `class` attribute showing up when not using a class.
- Fixed the empty `style` attribute showing up when using the `class` prop.
...
- Rewrite some descriptions in README.md

## 3.0.0
- Replaced `exclude` prop with `excludeByDOMNode` and `excludeByQuerySelector`.
...
## 2.5.4
- Replaced `on:click` with `on:mousedown`
- fixed #4
